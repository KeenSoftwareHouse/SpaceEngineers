...
void ComputeCommonOutput(VertexShaderInput input, float3 cellRelativePosition, float3 normal, float ambient, out VertexShaderOutput output)
{
...
VertexShaderOutput VertexShaderFunction(VertexShaderInput input)
{
    ...
    float ambientA = UnpackVoxelAmbient(input.PositionAndAmbient);
    float ambientB = UnpackVoxelAmbient(input.PositionMorph);
    ...
    float ambient = lerp(ambientA, ambientB, morph);

    VertexShaderOutput output;
    ComputeCommonOutput(input, position, normal, ambient, output);
    return output;
}
...
VertexShaderOutput_Multimaterial VertexShaderFunction_Multimaterial(VertexShaderInput input)
{
    ...
    output.Alpha = alpha;
    ComputeCommonOutput(input, position, normal, ambient, output.Single);
    return output;
}
