    ...
    {  
        ...
        public override bool DebugDraw()
        {
            if (MyDebugDrawSettings.DEBUG_DRAW_ROTORS)
            {
                ...
                m_motor.ComputeRotorQueryBox(out pos, out halfExtents, out orientation);
                ...
                {
                    var center = Vector3D.Transform(m_motor.DummyPosition, m_motor.CubeGrid.WorldMatrix) + (Vector3D.Transform(m_motor.Rotor.DummyPosLoc, m_motor.RotorGrid.WorldMatrix) - m_motor.RotorGrid.WorldMatrix.Translation);
