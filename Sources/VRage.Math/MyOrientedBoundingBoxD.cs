    ...
    public struct MyOrientedBoundingBoxD : IEquatable<MyOrientedBoundingBox>
    {
        ...
        public MyOrientedBoundingBoxD(MatrixD matrix)
        {
            Center = matrix.Translation;
             ...
             var scale = new Vector3D(matrix.Right.Length(), matrix.Up.Length(), matrix.Forward.Length());  //  matrix.Scale;
        ...
        {
            Vector3 mid = (box.Min + box.Max) * 0.5;
            Vector3 halfExtent = (box.Max - box.Min) * 0.5;
            return new MyOrientedBoundingBoxD(mid, halfExtent, Quaternion.Identity);
        }
        ...
        {
            return new MyOrientedBoundingBoxD(Vector3D.Transform(Center, rotation) + translation,
                                            HalfExtent,
        ...
        {
            return new MyOrientedBoundingBoxD(Vector3.Transform(Center * scale, rotation) + translation,
                                            HalfExtent * scale,
                                            Orientation * rotation);
        }
        ...
        public void Transform(MatrixD matrix)
        {
            ...
            Orientation = Quaternion.CreateFromRotationMatrix(MatrixD.CreateFromQuaternion(Orientation) * matrix);
        }
        ...
        {
            double dist = plane.DotCoordinate(Center);

            // Transform the plane's normal into this box's space
            Vector3D localNormal = Vector3D.Transform(plane.Normal, Quaternion.Conjugate(Orientation));
            ...
            // axes of the box.
            double r = Math.Abs(HalfExtent.X * localNormal.X)
        ...
        {
            Vector3D mB_T = mB.Translation;
            Vector3D mB_TA = new Vector3D(Math.Abs(mB_T.X), Math.Abs(mB_T.Y), Math.Abs(mB_T.Z));

            // Transform the extents of B
            Vector3D bX = mB.Right;      // x-axis of box B
            Vector3D bY = mB.Up;         // y-axis of box B
            Vector3D bZ = mB.Backward;   // z-axis of box B
            Vector3D hx_B = bX * hB.X;   // x extent of box B
            Vector3D hy_B = bY * hB.Y;   // y extent of box B
            Vector3D hz_B = bZ * hB.Z;   // z extent of box B

            // Check for containment first.
            double projx_B = Math.Abs(hx_B.X) + Math.Abs(hy_B.X) + Math.Abs(hz_B.X);
            double projy_B = Math.Abs(hx_B.Y) + Math.Abs(hy_B.Y) + Math.Abs(hz_B.Y);
            double projz_B = Math.Abs(hx_B.Z) + Math.Abs(hy_B.Z) + Math.Abs(hz_B.Z);
