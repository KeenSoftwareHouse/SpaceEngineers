...
float3 calculate_light(SurfaceInterface surface, float3 lightVector)
{
	...
	NL = max(NL*(1 - shadowMultiplier) + shadowMultiplier/M_PI, NL);

	return (1 - surface.emissive) * M_PI * NL * material_radiance(surface.albedo, surface.f0, surface.gloss, NE, lightVector, V, H);
}
