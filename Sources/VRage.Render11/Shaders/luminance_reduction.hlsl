	...
	uint ThreadIndex : SV_GroupIndex)
{
    ...
    if ( ThreadIndex == 0 ) {
        ...
        float adaptedlum = prevlum + (currentlum - prevlum) * (1 - exp(-frame_.tau * frame_.timedelta));

        float2 output = adaptedlum;
#else
        float2 output = ReduceBuffer[0];
#endif
        ReduceOutput[GroupID.xy] = Constant_luminance > 0 ? Constant_luminance : output;
    }
}
