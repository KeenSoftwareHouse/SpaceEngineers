...
void __pixel_shader(float4 svPos : SV_Position, out float4 output : SV_Target0
	...
	//output = clamp(ComputeAtmosphere(input.V, input.position, input.depth, input.native_depth, 10), 0, 10);
	output = ComputeAtmosphere(input.V, input.position, input.depth, input.native_depth, 4);
}
