...
float PhaseMie(float cosTheta) 
{
    const float g = MieG;
    const float gSq = g*g;
    return mad(1.5f, -gSq, 1.5f) * (1 + cosTheta * cosTheta) / (2.0f + gSq) / pow(abs(1.0f + gSq - 2.0f * g * cosTheta), 1.5f);
}
...
float4 ComputeAtmosphere(float3 inputV, float3 position, float depth, float native_depth, int steps)
{
	...
	if (depth_not_background(native_depth))
	{
		...
		totalInscatteringMie = 0;
	}
	float3 integral = (preIntegralRayleigh * totalInscatteringRayleigh + preIntegralMie * totalInscatteringMie);
	//return t;
	float atmosphereBoost = t * 2.5f * Intensity;
	return float4(integral * atmosphereBoost, saturate(rcp(PaPTransmittance.x * t)));
}
#endif