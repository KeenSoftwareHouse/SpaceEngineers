...
float4 shade_forward(SurfaceInterface surface, float3 position)
{
    ...
    shadow = 1 - (1 - shadow) * (1 - frame_.envShadowFadeout);

    float3 shaded = 0;
    shaded += surface.albedo * frame_.forwardPassAmbient;
    shaded += env_main_directional_light(surface, saturate(1 - frame_.forwardPassAmbient)) * shadow;
