...
VertexShaderInterface __prepare_interface(__VertexInput input, uint sv_vertex_id = 0, uint sv_instance_id = 0) {
    ...
    if ( facing > 0 )
    {
        __instance_matrix._41_42_43 += mul(centerOffset + voxelOffset, __instance_matrix);
        ...
        if ( facing == 3 )
        {
			...
			cameraDir = mul(cameraDir - position, t_local_matrix);
    ...
    if ( facing > 0 )
    {
        ...
        position += mul(voxelOffset + centerOffset, local_matrix);
    ...
    if ( windScale > 0 )
    {
        position_instance.xyz += windScale * sin(frame_.time * windFrequency + position.x + position_instance.x + centerOffset) * __normal;
    }
    ...
    if ( massiveRadius > 0 )
    {
        ...
        result.lDir.x = dot(ndelta, frame_.directionalLightVec);
    }
